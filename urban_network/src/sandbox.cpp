...
void dijkstra_updating_sim() {
    ...
    Iter_Vertex vstop = std::find_if(boost::vertices(g_dual).first, boost::vertices(g_dual).second, [&](Vertex dual_vertex) {return g_dual[dual_vertex].index == 22; });
    for (int i = 0; i < 7; i++) {
        boost::add_stationary_agents(g_dual, *vstop, *vf, dual_map, 120);
    }
...
void check_parser_dual(){
    ...
    for (auto it = boost::vertices(g_dual).first; it != boost::vertices(g_dual).second; it++) {
        int ix = g_dual[*it].index;
        auto itpv = std::find_if(boost::vertices(p).first, boost::vertices(p).second, [&](PVertex v) {return p[v].index == ix; });
        if (itpv == boost::vertices(g_dual).second) { std::cout << "out of bound;"; };

        auto range_dual = boost::out_edges(*it, g_dual);
        auto range_parser = boost::out_edges(*itpv, p);

        for (auto ut = range_dual.first; ut != range_dual.second; ut++) {
            Vertex ptd = boost::target(*ut, g_dual);
            int new_ix = g_dual[ptd].index;

            auto utpv = std::find_if(range_parser.first, range_parser.second, [&](PEdge e) {return p[boost::target(e,p)].index == new_ix; });
            std::cout << (utpv != range_parser.second);
        }
    }
}
