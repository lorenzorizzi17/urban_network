...
public:
    ...
    void save(Graph& m_dual) {
        ...
        for (int i = 0; i < m_index.size(); i++) {
            std::ofstream file;
            file.open("fig/occupation_vs_time/RW_T" + std::to_string(TIME_MAX_SIMULATION) + "_N" + std::to_string(m_N) + "_ix" + std::to_string(m_index[i]) + ".txt", std::ios::app);
        ...
        file_flux.open("fig/flux/RW_T" + std::to_string(TIME_MAX_SIMULATION) + "_N" + std::to_string(m_N) + ".txt", std::ios::app);
